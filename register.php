...
if(!empty($_POST['username']))
{
    ...
    $username = trim($_POST['username']); // 也可用mysql_real_escape_string()进行过滤
    ...
    // 插入数据
    try{
        $sql = "INSERT `im_user`(`username`,`password`,`created_time`) value('{$username}','{$password}','{$_SERVER['REQUEST_TIME']}')";
        $obj = $con->prepare($sql);
        $obj->execute();
    }catch (PDOException $e){
        die('操作失败'.$e->getMessage());
    }
    //INSERT `im_user`(`username`,`password`,`created_time`) value('ss','123456','146546');
    if($obj)
    {
